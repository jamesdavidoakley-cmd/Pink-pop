...
export const audio = {
  ...
  tap() { tone(520, 0, 0.08, 'sine', 0.12, 780); },
  place() { tone(300, 0, 0.1, 'triangle', 0.14, 200); },
  tip() { tone(240, 0, 0.25, 'sawtooth', 0.05, 160); },
  clink() { tone(1400, 0, 0.12, 'sine', 0.1, 1800); tone(2100, 0.05, 0.1, 'sine', 0.05); },
  sparkle() { [1047, 1319, 1568, 2093].forEach((f, i) => tone(f, i * 0.05, 0.25, 'sine', 0.08)); },
  brew() { noise(0, 0.9, 0.12); [262, 330, 392, 523, 659, 784].forEach((f, i) => tone(f, 0.2 + i * 0.09, 0.4, 'triangle', 0.14)); },
  whoosh() { noise(0, 0.4, 0.18); tone(300, 0, 0.4, 'sine', 0.06, 1200); },
  firework() { tone(500, 0, 0.5, 'sine', 0.05, 1500); noise(0.5, 0.45, 0.35); tone(80, 0.5, 0.4, 'sine', 0.2, 40); },
  fanfare() {
