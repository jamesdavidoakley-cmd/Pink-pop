...
console.log('after 12 cookies:', counts.join(','));
...
console.log('after open:', counts.join(','));
...
const rodChip = () => page.$$eval('#bp-chips .chip', (els) => els.map((e) => e.textContent.trim()).find((t) => /pack/.test(t)));
for (let i = 0; i < 10; i++) { await page.click(`${col(1)} .add`, { force: true }); await page.waitForTimeout(80); }
await idle();
let chip = await rodChip();
console.log('after 10 packs (snapped):', chip, '| counts', (await dbg()).counts.join(','));
if (chip !== '2 packs') errors.push(`tricky blueprint: expected "2 packs" after the fuse, got "${chip}"`);
for (let i = 0; i < 2; i++) { await page.click(`${col(1)} .add`, { force: true }); await page.waitForTimeout(80); }
await idle();
chip = await rodChip();
console.log('after 12 packs:', chip);
if (chip !== '0 packs') errors.push(`tricky blueprint: expected "0 packs", got "${chip}"`);
