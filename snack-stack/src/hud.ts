...
export class Hud {
  ...
  constructor() {
    ...
    for (const p of [0, 1, 2, 3] as Place[]) {
      ...
      col.innerHTML = `
        ...
        <div class="col-btns">
          ${p > 0 ? '<button class="smash" title="Open into ten">✂️</button>' : ''}
  ...
  showMenu(save: SaveData): void {
    ...
    $('city-count').textContent = save.towers ? `Shelf: ${save.towers} cake${save.towers === 1 ? '' : 's'} 🎂` : 'Finish a level to put a cake on the shelf.';
    $('menu').hidden = false;
  ...
  setChips(remaining: number[] | null, total?: number[]): void {
    ...
    for (let p = 3; p >= 0; p--) {
      ...
      chip.innerHTML = `<span class="sw" style="background: currentColor"></span>${remaining[p]} ${BLOCK_NAMES[p]}${remaining[p] === 1 ? '' : p === 2 ? 'es' : 's'}`;
      c.appendChild(chip);
    }
  }
